...
function Strategy(options, verify) {
  ...
  if (!options.customHeaders['User-Agent']) {
    options.customHeaders['User-Agent'] = options.userAgent || 'passport-github-scopes';
  }
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    var scopes = res.headers['x-oauth-scopes'] && res.headers['x-oauth-scopes'].split(", ");
    var hasEmailPermission = (scopes.indexOf('user:email') >= 0) || (scopes.indexOf('user') >= 0);
